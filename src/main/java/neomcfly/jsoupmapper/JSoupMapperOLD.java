...
public class JSoupMapperOLD {
    ...
    protected <T> Collection<T> parseList(InputStream in,
            Class<T> classArgument) {

        try {

            Document parse = Jsoup.parse(in, "", "/");
    ...
    protected void setValue(Object target, AccessibleObject member,
            Object value) {
        try {
            if (member instanceof Field) {
                setValue(target, (Field) member, value);
            } else if (member instanceof Method) {
                setValue(target, (Method) member, value);
            }
