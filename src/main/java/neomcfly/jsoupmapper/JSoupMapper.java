...
public class JSoupMapper {
	...
	protected static final String PACKAGE_NAME = JSoupSelect.class.getPackage().getName();
	...
	private <T> void map(Document document, Elements elements,
		...
		if (hasOneAnnotationMapper(member)) {
			...
			if (!optional && value == null) {
				...
				mapFieldOrArgument(document, elementsSelected, target, member, value);
			}
		}
	...
	protected void mapFieldOrArgument(Document document, Elements elements, Object target,
		...
		try {
			...
			if (member instanceof Field) {
				...
				Object values = evaluateArguments(document, elements, value, fieldType)[0];
				...
				Object[] values = evaluateArguments(document, elements, value, parameterTypes);
	 ...
	 * @param argumentsType
	 * 
	 * @return Arguments ordered giving method signature aka <code>argumentsType<code>
	 */
	protected Object[] evaluateArguments(Document document, Elements elements, Object value, Class<?>...argumentsType) {
		...
		for (int i = 0; i < argumentsType.length; i++) {
			...
			if(argumentType.isAssignableFrom(value.getClass())){
				arguments[i] = value;
			} else if(argumentType.isAssignableFrom(Elements.class)){
				arguments[i] = elements;
			} else if(argumentType.isAssignableFrom(Element.class)){
				if(elements.size() == 1){
					arguments[i] = elements.first();
				}else{
					throw new IllegalArgumentException("Cant set 'Element' object with Elements of size:"+elements.size());
				}
			} else if(argumentType.isAssignableFrom(Document.class)){
